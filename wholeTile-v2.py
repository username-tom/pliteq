...
def find_squares(img):
    ...
    for cnt in contours:
        ...
        if len(cnt) == 4 and cv.contourArea(cnt) > 0 and cv.isContourConvex(cnt):
            cnt = cnt.reshape(-1, 2)
...
def is_std_ramp(cnt):
    ...
    if x-y>y and (x/y)>2.5:   # horizontal
        return True
    elif y-x>x and (y/x)>2.5:    # vertical
        return True
    else:
        return False
...
def main(argv):
    ...
    if args.filename:
        ...
        ramps = find_squares(img)
        if len(ramps) > 0:
            cv.drawContours( img, ramps, -1, 100, 1 )
        else: print(args.filename + " can't be found")
        cv.imwrite(str(file) + '-' + str(len(ramps)) + '.png',img)   # Save to file name
